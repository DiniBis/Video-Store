...
/*s'il y a un réalisateur dans l'url, le récupérer*/
$uri = $_SERVER['REQUEST_URI'];
$uri = explode('/',$uri);
if (isset($uri[2])) {
    $realisateur= htmlspecialchars($uri[2]);
    /*évite les erreurs dûes aux caractères spéciaux dans l'url ex:Com%C3%A9die*/
    $realisateur = urldecode($realisateur);
} else {
    $realisateur=null;
}
/*Requête SQL*/
$requete = $pdo->prepare('SELECT id, titre, prix, realisateur, categorie, image, acteur FROM videos WHERE realisateur = :realisateur');
$requete->execute(['realisateur'=>$realisateur]);
